...
const STEPS = [
  ...
  { label: "Building report",         detail: "Compiling scores, findings and suspicious region markers" },
];
...
function VerdictBadge({ verdict }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
      ...
      {verdict.lowConf && <span style={{ fontSize: 11, color: "#854F0B" }}>Low confidence — treat with caution</span>}
    </div>
  );
}
...
export default function App() {
  ...
  const analyse = async () => {
    ...
    try {
      ...
      const parsed  = { ai: data.ai, df: data.df, verdict, claudeText: data.finding, zones: data.zones ?? [] };
      ...
      clearInterval(timerRef.current);
      setLoading(false);
      setResult({ ai: 0, df: 0, verdict: getVerdict(0, 0), claudeText: "Analysis failed: " + err.message, zones: [] });
    }
  };

  const reset = () => {
    setImgUrl(null); setImgB64(null); setResult(null);
    setShowHeat(false); setLoading(false); setRevealing(false); setStep(0);
  };
  ...
  // Reveal card meta
  const revealScore = result ? Math.max((result.ai ?? 0), (result.df ?? 0)) * 100 : 0;
  const revealMeta  = revealScore >= 70
                  ...
                  <div style={{ position: "absolute", inset: 0, borderRadius: 8 }}>
                    {result.zones.map((z, i) => (
                      <div key={i} style={{ position: "absolute", left: `${z.left}%`, top: `${z.top}%`, width: `${z.width}%`, height: `${z.height}%`, borderRadius: 4, border: "1.5px solid rgba(226,75,74,0.65)", background: "rgba(226,75,74,0.13)" }}>
                        <span style={{ position: "absolute", bottom: "100%", left: 0, marginBottom: 3, fontSize: 9, color: "rgba(255,255,255,0.95)", background: "rgba(163,45,45,0.8)", borderRadius: 3, padding: "1px 5px", whiteSpace: "nowrap", maxWidth: 130, overflow: "hidden", textOverflow: "ellipsis" }}>
                          {z.label}
                        </span>
                      </div>
                    ))}
              ...
              <>
                ...
                <div style={card}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <button onClick={copyAnalysis} style={{ fontSize: 11, padding: "3px 10px", cursor: "pointer", color: copied ? "#27500A" : "#5F5E5A", borderColor: copied ? "#C0DD97" : undefined, background: copied ? "#EAF3DE" : undefined }}>
                      {copied ? "Copied" : "Copy"}
                    </button>
                  </div>
                  <p style={{ fontSize: 13, lineHeight: 1.75, color: "#2c2c2a", margin: 0 }}>{result.claudeText}</p>
                </div>
